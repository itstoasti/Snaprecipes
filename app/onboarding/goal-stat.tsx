...
export default function GoalStatScreen() {
    const router = useRouter();
    const { state } = useOnboarding();

    const selectedGoals = state.goals.length ? state.goals : ["eat_healthier"];
    ...
    const handleContinue = () => {
        if (selectedGoals.length > 1) {
            router.push("/onboarding/goal-affirmation");
        } else {
    ...
    return (
        <View className="flex-1 bg-[#FAF7F2] justify-between">
            <OnboardingHeader progress={0.28} />
            ...
            <View className="flex-1 px-6 pt-4 pb-8 justify-between">
                ...
                <Animated.View entering={FadeIn.duration(600)} className="space-y-3 items-center">
                    ...
                    <Text className="text-[#4B5563] font-sans text-xl text-center leading-relaxed px-1">
                        92% of users report that SnapRecipes has seamlessly helped them to{" "}
                        <Text className="font-sans-bold text-[#1F2937]">{formattedGoalString}</Text>
                    </Text>
                </Animated.View>
                ...
                >
                    <View className="w-[330px] h-[330px] items-center justify-center shadow-xl shadow-black/10">
                        <Svg width={330} height={330} viewBox="0 0 200 190">
                            ...
                            <SvgImage
                                href={require("../../assets/recime_lifestyle_1.jpg")}
                                width="200"
                                height="190"
